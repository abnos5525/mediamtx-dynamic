...
type streamStart struct {
	...
	OutputType int    `json:"output_type" example:"7"`
...
func Spec() []byte {
	...
	r.SpecEns().Info.WithTitle("mediamtx-dynamic").WithVersion("1.0.0").
		WithDescription("MediaMTX video plus AI overlay sniff. output_type 7 returns only the WebRTC WHEP url. output_type 6 returns only the LL-HLS url.")
	...
	st.SetSummary("Start MediaMTX path and AI sniff. 6 is HLS, 7 is WHEP")
